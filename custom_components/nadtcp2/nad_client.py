"""Async TCP/IP client for NAD C338 amplifiers."""
import asyncio
import logging
import socket
import time
...
class NADReceiverTCPC338(asyncio.Protocol):
    """asyncio Protocol that talks to a NAD C338 over TCP and tracks state."""
    ...
    def make_command(command, operator, value=None):
        ...
        # validate operator
        if operator not in cmd_desc["supported_operators"]:
        ...
        return command + operator + str(value)
    ...
    def data_received(self, data):
        ...
        while "\r\n" in self._buffer:
            ...
            # a volume change implicitly disables mute
    ...
    async def exec_command(self, command, operator, value=None):
        if self._transport:
            ...
            cmd_wait_time = (self._last_cmd_time
                             + self.CMD_MIN_INTERVAL) - time.time()
            if cmd_wait_time > 0:
